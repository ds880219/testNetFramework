namespace bel.web.api.core.Imaging.BitmapEffects
{
    ...
    public class Bitmap32
    {
        ...
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            blue = this.ImageBytes[i++];
            green = this.ImageBytes[i++];
            red = this.ImageBytes[i++];
            alpha = this.ImageBytes[i];
        }

        public void SetPixel(int x, int y, byte red, byte green, byte blue, byte alpha)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            this.ImageBytes[i++] = blue;
            this.ImageBytes[i++] = green;
            this.ImageBytes[i++] = red;
            this.ImageBytes[i] = alpha;
        }
        ...
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            return this.ImageBytes[i + 1];
        }

        public void SetGreen(int x, int y, byte green)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            this.ImageBytes[i + 1] = green;
        }

        public byte GetRed(int x, int y)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            return this.ImageBytes[i + 2];
        }

        public void SetRed(int x, int y, byte red)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            this.ImageBytes[i + 2] = red;
        }

        public byte GetAlpha(int x, int y)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            return this.ImageBytes[i + 3];
        }

        public void SetAlpha(int x, int y, byte alpha)
        {
            int i = y * this.m_BitmapData.Stride + x * 4;
            this.ImageBytes[i + 3] = alpha;
        }
        ...
        public void LockBitmap()
        {
            ...
            this.m_BitmapData = this.Bitmap.LockBits(bounds,
                ImageLockMode.ReadWrite,
                PixelFormat.Format32bppArgb);
            this.RowSizeBytes = this.m_BitmapData.Stride;

            // Allocate room for the data.
            int total_size = this.m_BitmapData.Stride * this.m_BitmapData.Height;
            this.ImageBytes = new byte[total_size];

            // Copy the data into the ImageBytes array.
            Marshal.Copy(this.m_BitmapData.Scan0, this.ImageBytes, 0, total_size);
        ...
        public void UnlockBitmap()
        {
            ...
            // Copy the data back into the bitmap.
            int total_size = this.m_BitmapData.Stride * this.m_BitmapData.Height;
            Marshal.Copy(this.ImageBytes, 0, this.m_BitmapData.Scan0, total_size);

            // Unlock the bitmap.
            this.Bitmap.UnlockBits(this.m_BitmapData);
        ...
        public void Average()
        {
            ...
            for (int y = 0; y < this.Height; y++)
            {
                for (int x = 0; x < this.Width; x++)
                {
                    ...
                    this.GetPixel(x, y, out red, out green, out blue, out alpha);
                    byte gray = (byte)((red + green + blue) / 3);
                    this.SetPixel(x, y, gray, gray, gray, alpha);
                }
            }
        ...
        public void Invert()
        {
            ...
            for (int y = 0; y < this.Height; y++)
            {
                for (int x = 0; x < this.Width; x++)
                {
                    byte red   = (byte)(255 - this.GetRed(x, y));
                    byte green = (byte)(255 - this.GetGreen(x, y));
                    byte blue  = (byte)(255 - this.GetBlue(x, y));
        ...
        public class Filter
        {
            ...
            {
                this.Weight = 0;
                for (int row = 0; row <= this.Kernel.GetUpperBound(0); row++)
                {
                    for (int col = 0; col <= this.Kernel.GetUpperBound(1); col++)
                    {
                        this.Weight += this.Kernel[row, col];
                    }
                }
            }
            ...
            public void ZeroKernel()
            {
                ...
                for (int row = 0; row <= this.Kernel.GetUpperBound(0); row++)
                {
                    for (int col = 0; col <= this.Kernel.GetUpperBound(1); col++)
                    {
                        total += this.Kernel[row, col];
                    }
                }

                int row_mid = (int)(this.Kernel.GetUpperBound(0) / 2);
                int col_mid = (int)(this.Kernel.GetUpperBound(1) / 2);
                total -= this.Kernel[row_mid, col_mid];
                this.Kernel[row_mid, col_mid] = -total;
            }
        }
                ...
                return new Filter()
                           {
                               ...
                               // {0,  0, -1},
                               // }
                               Kernel = new float[,] { { 2, 1, 0 }, { 1, 0, -1 }, { 0, -1, -2 } }, // Matrix3
                           };
            }
        }
        ...
        public Bitmap32 ApplyFilter(Filter filter, bool lock_result)
        {
            ...
            int xmin = -xoffset;
            int xmax = this.Bitmap.Width - filter.Kernel.GetUpperBound(1);
            int ymin = -yoffset;
            int ymax = this.Bitmap.Height - filter.Kernel.GetUpperBound(0);
            ...
            for (int x = xmin; x <= xmax; x++)
            {
                for (int y = ymin; y <= ymax; y++)
                {
                    ...
                    if (!skip_pixel)
                    {
                        ...
                        red = filter.RedOffset + red / filter.Weight;
                        if (red < 0) red = 0;
                        if (red > 255) red = 255;

                        green = filter.GreenOffset + green / filter.Weight;
                        if (green < 0) green = 0;
                        if (green > 255) green = 255;

                        blue = filter.BlueOffset + blue / filter.Weight;
                        if (blue < 0) blue = 0;
        ...
        public Bitmap32 RankMaximum(int rank, bool lock_result)
        {
            ...
            for (int y = 0; y < this.Height; y++)
            {
                for (int x = 0; x < this.Width; x++)
                {
                    ...
                    int best_brightness = this.GetRed(x, y) + this.GetGreen(x, y) + this.GetBlue(x, y);
                    int best_row = y;
                    int best_col = x;
                        ...
                        if ((row >= 0) && (row < this.Height))
                        {
                            for (int dx = start_i; dx < stop_i; dx++)
                            {
                                ...
                                if ((col >= 0) && (col < this.Width))
                                {
                                    int test_brightness =
                                        this.GetRed(col, row) +
                                        this.GetGreen(col, row) +
                                        this.GetBlue(col, row);
                    ...
                    result.SetPixel(x, y,
                        this.GetRed(best_col, best_row),
                        this.GetGreen(best_col, best_row),
                        this.GetBlue(best_col, best_row),
                        255);
                }
            }
        ...
        public void Pixellate(int rank, bool lock_result)
        {
            ...
            for (int y = 0; y < this.Height; y += rank)
            {
                for (int x = 0; x < this.Width; x += rank)
                {
                    ...
                    for (int row = y; row < y + rank; row++)
                    {
                        if (row < this.Height)
                        {
                            for (int col = x; col < x + rank; col++)
                            {
                                if (col < this.Width)
                                {
                                    total_r += this.GetRed(col, row);
                                    total_g += this.GetGreen(col, row);
                                    total_b += this.GetBlue(col, row);
                                    num_pixels++;
                                }
                            }
                        }
